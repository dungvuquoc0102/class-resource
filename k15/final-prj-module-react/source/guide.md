...
Clone Threads UI trong 3 tuần.

## Công Nghệ Sử Dụng

- Dựng dự án: Vite, ReactJS
...
- Toast mesage với Shadcn/Sonner
- Logging error với Sentry
- Deploy với Vercel
- Biến môi trường: .env, .env.production, .env.example
  ...
  - NavigationBar
    - UI
      - Mobile:
        ...
        - Kích thước và màu sắc: `w-dvw h-[50px]`
        - Text và các thuộc tính ảnh hưởng tới con của nó: `flex`
        - Dark mode:
        - Đa ngôn ngữ text tĩnh:
      - Desktop:
        - Vị trí (tuyệt đối: sát mép trái viewport): `md:right-auto md:top-0`
        - Kích thước và màu sắc: `md:w-19 md:h-dvh`
        - Text và các thuộc tính ảnh hưởng tới con của nó
        - Dark mode:
        - Đa ngôn ngữ text tĩnh:
    - Logic
